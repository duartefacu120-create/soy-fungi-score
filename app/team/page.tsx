...
export default async function TeamPage({ searchParams }: { searchParams: { message?: string } }) {
    ...
    async function handleRemoveMember(formData: FormData) {
        ...
        try {
            await removeMemberFromOrganization(id);
            redirect(`/team?message=Miembro eliminado de la empresa.`);
        } catch (e: any) {
            redirect(`/team?message=${encodeURIComponent(e.message)}`);
        }
    }
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        {!isOwner && (
                            <section className="bg-red-50 rounded-2xl border border-red-100 p-6">
                                ...
                                <p className="text-xs text-red-600 mb-4 font-medium">Si abandonas la empresa perderás el acceso a todos los datos compartidos (lotes, evaluaciones).</p>
                        ...
                        <section className="bg-white rounded-2xl border border-red-100 p-6 shadow-sm">
                            ...
                            <p className="text-[10px] text-gray-500 mb-4 leading-relaxed font-medium">
                                Esta acción es **permanente**. Se eliminará tu perfil y **todos los datos que hayas creado**. Si eres dueño, la empresa completa será eliminada.
                            </p>
                            <DeleteAccountForm />
                        </section>
                        ...
                        {isOwner && (
                            <section className="bg-gray-100 rounded-2xl p-6 border border-gray-200">
                                ...
                                <p className="text-[10px] text-gray-500 font-medium italic leading-relaxed">Como creador de la empresa, puedes gestionar miembros y la identidad de la compañía. Los dueños no pueden abandonar la empresa sin antes transferir la propiedad.</p>
                            </section>
                        )}
                    </div>
                                    ...
                                    {pendingInvitations.map((inv) => (
                                        <div key={inv.id} className="p-4 rounded-xl bg-white/10 backdrop-blur-md border border-white/20">
                                            <p className="text-sm mb-3">
                                                Te han invitado a unirte a <span className="font-bold underline">{inv.organization?.name}</span>
                                            </p>
                            ...
                            <div className="p-6 border-b bg-gray-50/50 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500">Colaboradores con acceso a los datos de la empresa</p>
                                    </div>
                                </div>
                                ...
                                {members.map((member) => (
                                    <div key={member.id} className="p-6 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {member.id !== user.id && (
                                                <div className="flex items-center gap-4">
                                                    {isOwner && member.id !== user.organization?.owner_id && (
